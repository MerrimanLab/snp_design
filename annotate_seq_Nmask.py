...
def annotate_nearby(all_vcf, position_snp, sequence, mask_character):
    """
...
def write_snp_design(sequence, position, chromosome, output_file):
    """
...
def main():
    ...
    assert args.mask_character == "iupac" or args.mask_character == "N", "--mask requires either iupac or N as the input argument"
    if args.output_file == None:
    	args.output_file = chromosome + "." + position
    ...
    sequence = annotate_nearby(subset_vcf, args.position, sequence, args.mask_character)
    sequence = finalise_design(sequence, args.alternate_allele)
    write_snp_design(sequence,args.position, args.chromosome, args.output_file)  
